...
Aqua Chemistry requires a computational chemistry program or library, known as *driver*, to be installed on the
...
In order for a driver to be usable by Aqua Chemistry, an interface to that driver
must be built in Aqua Chemistry.  Aqua Chemistry offers the ``BaseDriver``
...
.. topic:: The HDF5 Driver
    ...
    A fifth driver, called HDF5, comes prebuilt in Aqua Chemistry.  This is, in fact, the only driver
...
.. topic:: Extending Aqua Chemistry with Support for New Drivers

    The driver support in Aqua Chemistry was designed to make the drivers pluggable and discoverable.
...
If no prebuilt native extension binary, as supplied with Aqua Chemistry, works for your platform, then
...
.. topic:: Special Notes for macOS X
    ...
    Now, before Aqua Chemistry can properly interface Gaussian™ 16, you will have to run the ``enable_gaussian`` command
...
To use Gaussian™ 16 to configure a molecule on which to do a chemistry experiment with Aqua Chemistry,
...
In order for Aqua Chemistry to discover PSI4 at run time, it is then necessary to execute the ``enable_psi4`` command
before launching Aqua Chemistry.

To use PSI4 to configure a molecule on which to do a chemistry experiment with Aqua Chemistry,
...
by Aqua Chemistry at run time.

To use PySCF to configure a molecule on which to do a chemistry experiment with Aqua Chemistry,
...
make PyQuante2 dynamically discovered by Aqua Chemistry at run time.

The Aqua Chemistry PyQuante2 driver wrapper contains two methods, in ``transform.py``, taken from from
...
.. note::
    Like all the other drivers currently interfaced by Aqua Chemistry,
...
To use PyQuante to configure a molecule on which to do a chemistry experiment with Aqua Chemistry,
...
Aqua Chemistry uses a molecular input file written on top of one of the classical computational software drivers
...
the installation of Aqua Chemistry itself.
...
Upon completing its execution, Aqua Chemistry displays the following message:
...
an :ref:`aqua-chemistry-input-file` and then invoking the Aqua Chemistry
