...
def calculate_correction(num,n,p):
...
def get_approximation_inv(num,n,p):
    ...
    correction = calculate_correction(diff,n,p)
    #print(nsmallest,correction)
...
def generate_circuit(circuit_info,decimals,accuracy,qc=None):
    ...
    for floatingnumber,(in_pat,out_pat) in zip(circuit_info.keys(),circuit_info.values()):
        ...
        if last_max_exp != max_exp:
            ...
            for i in range(2 + _, min(1 + _ + accuracy,n)):
            ...
            qc.ccx(flagbit[0], anc[i - 1 - _], outputregister[n - 1 - _])
            qc.x(flagbit)
            #print("works")
            # uncompute
            for i in range(min( _ + accuracy,n - 1), 1 + _, -1):
                qc.x(inputregister[i])
                qc.ccx(inputregister[i], anc[i - 2 - _], anc[i - 1 - _])
                qc.x(inputregister[i])
            qc.x(inputregister[_ + 1])
...
def design_correction_circuit(n_in,decimals,accuracy,qc):
    ...
    for max_exp in np.arange(n_in-1,accuracy-1,-1):#,accuracy-1,-1):

        for n, i in enumerate(map(''.join, itertools.product('01', repeat=accuracy))):
            vec = [0]*n_in
        ...
        for dem_accuracy in range(accuracy,1,-1):
