...
class TestQSVM(QiskitAquaTestCase):
    ...
    def setUp(self):
        ...
        self.ref_kernel_testing = np.array([[0.14316667, 0.18208333, 0.4785, 0.14441667],
        ...
        # multiclass input
        self.multi_training = {'A': np.asarray([[0.6560706, 0.17605998],
        ...
        self.multi_test = {'A': np.asarray([[0.57483139, 0.47120732],
    ...
    def test_qsvm_deprecated_feature_maps(self):
        """Test QSVM on FeatureMap types."""
        ref_kernel_training = self.ref_kernel_training
        ref_kernel_testing = self.ref_kernel_testing
        ref_support_vectors = self.ref_support_vectors
        ...
        data_preparation = SecondOrderExpansion(2, depth=2, entangler_map=[[0, 1]])
    ...
    def test_qsvm_binary(self):
        ...
        backend = BasicAer.get_backend('qasm_simulator')
        data_preparation = ZZFeatureMap(2, reps=2)
        svm = QSVM(data_preparation, self.training_data, self.testing_data, None)
        quantum_instance = QuantumInstance(backend,
                                           shots=self.shots,
                                           seed_simulator=self.random_seed,
    ...
    def test_qsvm_multiclass_one_against_all(self):
        ...
        training_input = self.multi_training
        test_input = self.multi_test
