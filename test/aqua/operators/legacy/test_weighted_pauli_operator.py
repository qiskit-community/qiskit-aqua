...
class TestWeightedPauliOperator(QiskitAquaTestCase):
    ...
    def test_from_to_file(self):
        """ from to file test """
        paulis = [Pauli(label=x) for x in ['IXYZ', 'XXZY', 'IIZZ', 'XXYY', 'ZZXX', 'YYYY']]
    ...
    def test_str(self):
        ...
        coeff_a = 0.5
        coeff_b = 0.5
        pauli_term_a = [coeff_a, Pauli(label=pauli_a)]
        pauli_term_b = [coeff_b, Pauli(label=pauli_b)]
    ...
    def test_iadd(self):
        ...
        pauli_c = 'IXYZ'
        coeff_c = 0.25
        pauli_term_c = [coeff_c, Pauli(label=pauli_c)]
    ...
    def test_evaluate_single_pauli_qasm(self):
        ...
        # X
        op = WeightedPauliOperator.from_list([Pauli(label='X')])
