...
Examples::
        ...
        prob_grad = Gradient(grad_method='fin_diff').convert(operator=CircuitStateFn(
                                                             primitive=qc, coeff=1.), \
                                                             params=[a])
...
Examples::
        ...
        qfi = QFI('lin_comb_full').convert(operator=CircuitStateFn(primitive=qc, coeff=1.), \
                                           params=[a])
...
callable and are thus compatible with the optimizers from `qiskit.aqua.components.optimizers`.
