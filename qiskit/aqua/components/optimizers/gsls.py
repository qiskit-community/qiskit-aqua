...
class GSLS(Optimizer):
    ...
    def __init__(self,
        ...
        sample_size_factor : The size of the sample set at each
                             iteration is this number multiplied by
                             the dimension of the problem, rounded to
                             the nearest integer.
        initial_step_size : Initial step size for the descent
                            algorithm.
        min_step_size : Minimum step size for the descent algorithm.
        step_size_multiplier : Step size reduction after unsuccessful
                               steps, in the interval (0, 1).
        armijo_parameter : Armijo parameter for sufficient decrease
                           criterion, in the interval (0, 1).
        min_gradient_norm : If the gradient norm is below this
                            threshold, the algorithm stops.
        max_failed_rejection_sampling : Maximum number of attempts to
    ...
    def optimize(self, num_vars, objective_function, gradient_function=None,
        ...
        if variable_bounds is None:
            var_lb = np.array([float('-inf')] * num_vars)
            var_ub = np.array([float('inf')] * num_vars)
        else:
            var_lb = np.array([l for (l, u) in variable_bounds])
            var_ub = np.array([u for (l, u) in variable_bounds])

        x, x_value, n_evals, grad_nom = self.ls_optimize(
            num_vars, objective_function, initial_point, var_lb, var_ub)

        return x, x_value, n_evals

    def ls_optimize(self, n, obj_fun, initial_point, var_lb, var_ub):
        ...
        initial_point : Initial point. Must be a Numpy array.
        var_lb : Vector of lower bounds on the decision
                 variables. Vector elements can be float('-inf') if
                 the corresponding variable is unbounded from
                 below. Must be a Numpy array.
        var_ub : Vector of upper bounds on the decision
                 variables. Vector elements can be float('inf') if
                 the corresponding variable is unbounded from
                 below. Must be a Numpy array.

        Returns: Final iterate as a vector, corresponding objective
            function value, number of evaluations, and norm of the
            gradient estimate.

        """
        assert(len(initial_point) == n)
        assert(len(var_lb) == n)
        assert(len(var_ub) == n)
        ...
        while (iter_count < self._options['max_iter'] and
            ...
            grad = self.gradient_approximation(n, x, x_value, u,
                                               sample_set_x, sample_set_y)
            grad_norm = np.sqrt(grad.dot(grad))
            d = grad
            new_x = np.clip(x - alpha * d, var_lb, var_ub)
            ...
            if (new_x_value <= x_value -
                ...
                prev_u = u
                prev_sample_set_x = sample_set_x
    ...
    def sample_set(self, n, x, var_lb, var_ub, num_points):
        """Construct sample set of given size.

        Args:
        n : Dimension of the problem.
        x : Point around which the sample set is constructed, as a 1D
            numpy.ndarray[float]
        var_lb : Vector of lower bounds on the decision
                 variables. Vector elements can be float('-inf') if
                 the corresponding variable is unbounded from
                 below. Must be a 1D numpy.ndarray[float].
        var_lb : Vector of upper bounds on the decision
                 variables. Vector elements can be float('inf') if
                 the corresponding variable is unbounded from
                 below. Must be a 1D numpy.ndarray[float].
        num_points : Number of points in the sample set.

        Returns: Matrices of (unit-norm) sample directions and sample
                 points, one per row. Both matrices are 2D
                 numpy.ndarray[float].
        ...
        if ((points >= var_lb).all() and (points <= var_ub).all()):
            ...
            return directions, (x + self._options['sampling_radius'] *
                                directions)
        else:
            ...
            while (len(accepted) < num_points and
                ...
                num_trials += 1
            # When we are at a corner point, the expected fraction of
            # acceptable points may be exponential small in the
            # dimension of the problem. Thus, if we keep failing and
            # do not have enough points by now, we switch to a
            ...
            return (accepted[:num_points],
                    x + self._options['sampling_radius'] *
                    accepted[:num_points])
    # -- end function

    def gradient_approximation(self, n, x, x_value, directions,
                               sample_set_x, sample_set_y):
        ...
        n : Dimension of the problem.
        x : Point around which the sample set was constructed, as a 1D
            numpy.ndarray[float].
        x_value : Objective function value at x.
        directions : Directions of the sample points wrt the central
                     point x, as a 2D numpy.ndarray[float].
        sample_set_x : x-coordinates of the sample set, one point per
                       row, as a 2D numpy.ndarray[float].
        sample_set_y : Objective function values of the points in
                      sample_set_x, as a 1D numpy.ndarray[float].

        Returns: Gradient approximation at x, as a 1D numpy.ndarray[float].

        """
        ffd = (sample_set_y - x_value)
        gradient = ((float(n) / len(sample_set_y)) * np.sum(
