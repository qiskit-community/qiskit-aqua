...
class ORBIT(Optimizer):
    """ORBIT algorithm.
    """

    CONFIGURATION = {
        'name': 'ORBIT',
        'description': 'ORBIT Optimizer',
        'input_schema': {
    ...
    def optimize(self, num_vars, objective_function, gradient_function=None, variable_bounds=None, initial_point=None):
        ...
        logger.warning("ORBIT is supported only in Python3. If it does not run."
                     ...
                     "Rcpp' can be installed from commandline."
                     "Previously to that, you need to download CRAN OSX Binaries for Mac."
                     "For this also need to have a gfortran."
                     "Install it with 'brew install gcc' (install Homebrew first).")

        res, history = skq.minimize(objective_function, np.array(initial_point), bounds=np.array(variable_bounds),
                                    budget=self._maxiter, method="orbit")
