...
def run(operator, optimizer, varform, chosen_backend, num_qubits, initialParameters=None,
        shotsPerPoint=10000):
    global shots
    global values
    global plottingTime
...
def opToCircs(circuit=QuantumCircuit, operator=WeightedPauliOperator, qr_size=int):
    ...
    meascircuits = operator.construct_evaluation_circuit(circuit, statevector_mode=False, qr=None,
                                                         cr=None, use_simulator_operator_mode=False,
                                                         circuit_name_prefix='')
    ...
    for j in range(numRegisters):
        m = j*paulis_per_register
        for k in range(paulis_per_register-1):
            if(m+k+1 < len(qr)):
                meascircuits[m].add_register(qr[j*(paulis_per_register-1)+k+1],
                                             cr[j*(paulis_per_register-1)+k+1])
                meascircuits[m].append(meascircuits[m+k+1].to_instruction(),
                                       qr[j*(paulis_per_register-1)+k+1],
                                       cr[j*(paulis_per_register-1)+k+1])
        output_circ.append(meascircuits[m].decompose())
    return output_circ


# Function that is used by the optimizer, takes parameters and returns respective energy
def energy_opt(parameters):
    # Build circuit with parameters corresponding to the variational form
