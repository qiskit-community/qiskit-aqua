...
from qiskit.aqua.circuits.linear_z_rotation import LinearZRotation as LinRZ
import numpy as np


class PiecewiseLinearZRotation(CircuitFactory):
    """
    ...
    def build(self, qc, q, q_ancillas=None):
        ...
        for i, bp in enumerate(self.breakpoints):
            ...
            if i == 0 and self.contains_zero_breakpoint:
                ...
                # apply rotation
                lin_rz = LinRZ(self.mapped_slopes[i], self.mapped_offsets[i], self.num_state_qubits, i_state=i_state, i_target=i_target)
                lin_rz.build(qc, q)
                ...
                lin_rz = LinRZ(self.mapped_slopes[i], self.mapped_offsets[i], self.num_state_qubits, i_state=i_state, i_target=i_target)
                lin_rz.build_controlled(qc, q, q_ancillas[i - 1], use_basis_gates=False)
                ...
                # apply controlled rotation
                lin_rz = LinRZ(self.mapped_slopes[i], self.mapped_offsets[i], self.num_state_qubits, i_state=i_state, i_target=i_target)
                lin_rz.build_controlled(qc, q, q_ancillas[i], use_basis_gates=False)
