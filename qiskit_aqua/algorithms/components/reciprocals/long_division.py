...
class LongDivision(Reciprocal):
    ...
    def _ld_circuit(self):
        ...
        def shift_to_one(qc,b, anc, n):           
            ...
            for j2 in range(n-2):           #if msb is 1, change ancilla j2 to 0
                qc.cx(b[n-2], anc[j2])                            
                for i in  np.arange(n-2,0, -1):     #if msb was 0, anc1[j2] is ...,  
        ...
        for ish in range(self._n-2):                      #unshifting due to initial alignment
            ...
            shift_one_rightc(self._circuit, self._ev, self._anc1[ish], self._n-1)
            self._circuit.x(self._anc1[ish])
    ...
    def construct_circuit(self, mode, inreg):
        ...
        self._precision = int(self._precision)
        ...
        self._anc1 = QuantumRegister(self._num_ancillae, 'aligning_ancilla')  #ancilla for the initial shifting
        ...
        for i in range(floor(0.5*self._num_ancillae)):
            i = int(i)
            qc.swap(self._ev[i], self._ev[self._num_ancillae -i-1])
