...
class InputParser(object):
    ...
    def __init__(self, input=None):
        ...
        self._section_order = [JSONSchema.PROBLEM,InputParser.INPUT,JSONSchema.ALGORITHM]
    ...
    def _update_algorithm_input_schema(self):
        ...
        default_name = self.get_property_default_value(InputParser.INPUT,JSONSchema.NAME)
        input_name = self.get_section_property(InputParser.INPUT,JSONSchema.NAME,default_name)
        ...
        if InputParser.INPUT not in self._json_schema.schema['properties']:
            self._json_schema.schema['properties'][InputParser.INPUT] = { 'type': 'object' }
    ...
    def _update_input_problem(self):
        ...
        for input_name in local_inputs():
            if problem_name in self.get_input_problems(input_name):
                # set to the first input to solve the problem
                self.set_section_property(InputParser.INPUT,JSONSchema.NAME,input_name)
                return
