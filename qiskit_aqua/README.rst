...
.. topic:: Extending the Algorithm Library
    ...
    existing algorithms.  Aqua also allows for new components to register themselves as Aqua extensions and be dynamically
...
speedups. All that is needed for carrying out a search is an oracle oracles for
specifying the search criterion, which basically indicates a hit or miss
for any given record.

.. seealso::
    Consult the documentation on :ref:`oracles` for more details.

Currently the satisfiability (SAT) oracle
implementation is provided, which takes as input an SAT problem in
`DIMACS CNF
...
.. topic:: Problems Supported
   ...
   In Aqua, Grover supports the ``search`` problem.
