...
from qiskit_aqua_chemistry.ui._customwidgets import EntryPopup, ComboboxPopup, TextPopup
import psutil
import os
import subprocess
import threading
...
class Controller(object):
    ...
    def export_dictionary_to_clipboard(self,window):
        try:
            value = json.loads(json.dumps(self._model.get_dictionary()))
    ...
    def export_dictionary_to_file(self,filename):
        try:
            self._model.export_dictionary(filename)
            self._outputView.write_line("Exported to file: {}".format(filename))
            return True
        except Exception as e:
            messagebox.showerror("Error",str(e))
...
class AquaChemistryThread(threading.Thread):
    ...
    def run(self):
        ...
        temp_input = False
        try:
            aqua_chemistry_directory = os.path.dirname(os.path.realpath(__file__))
            aqua_chemistry_directory = os.path.abspath(os.path.join(aqua_chemistry_directory,'..'))
